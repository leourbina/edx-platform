...
class VideoPage(PageObject):
    ...
    def set_speed(self, speed):
        ...
        EmptyPromise(lambda: self.q(css='.speeds').visible, 'Video Speed set to '.format(speed)).fulfill()
    ...
    def can_we_download_transcript(self, transcript_format, text_to_search):
        ...
        if '.' + transcript_format not in self.q(css=VIDEO_MENUS['transcript-format']).text[0]:
            return False
    ...
    def select_language(self, code):
        ...
        # self._wait_for_element('.lang.open', 'CC Menu Shown')
        ...
        if len(self.q(css=VIDEO_MENUS["language"] + ' li.is-active').results) != 1:
            return False
